"""
Train TC — Triplet Contrastive (Paper Table 1, Row 2)
=====================================================
Supervision : Supervised
Objective   : Margin-based triplet loss
View        : FullView
Granularity : Table
Base model  : all-mpnet-base-v2

Triplets: anchor table, positive (FK-linked or same-DB), negative (different DB).
"""
...
from taberta.data_preparation import prepare_tc_triplets
from taberta.config import TCConfig
from taberta.metrics import MetricsLogger

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(message)s",
    handlers=[logging.FileHandler("train_TC.log"), logging.StreamHandler()],
)
...
def main():
    parser = argparse.ArgumentParser(description="Train TC — Triplet Contrastive")
    ...
    logger.info(f"TC Training | device={device} | epochs={cfg.epochs} | triplets={cfg.target_triplets}")
    ...
    triplets = prepare_tc_triplets(
        ...
        use_fk_positives=True, seed=cfg.seed, max_databases=args.max_databases,
    )
    ...
    ml = MetricsLogger(metrics_csv, strategy="TC")
    ...
    logger.info(f"TC model saved to {cfg.output_dir}")
