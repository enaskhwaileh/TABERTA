"""
Train TC-SB — Triplet Contrastive SmartBatch (Paper Table 1, Row 3)
===================================================================
Supervision : Supervised
Objective   : Margin-based triplet loss + smart batching
View        : FullView
Granularity : Table
Base model  : all-mpnet-base-v2

Same-DB positives with smart batching: mini-batches group semantically
related tables to ensure competitive negatives. Includes early stopping
and cosine-annealing LR schedule.
"""
...
from taberta.data_preparation import prepare_tc_sb_triplets
from taberta.config import TCSBConfig
from taberta.metrics import MetricsLogger

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(message)s",
    handlers=[logging.FileHandler("train_TC_SB.log"), logging.StreamHandler()],
)
...
def main():
    parser = argparse.ArgumentParser(description="Train TC-SB — Triplet Contrastive SmartBatch")
    ...
    logger.info(f"TC-SB Training | device={device} | epochs={cfg.epochs} | patience={cfg.patience}")

    # ---- Data ----
    corpus = WikiDBsCorpus(args.data_dir, row_limit=cfg.row_limit)
    logger.info(f"Corpus: {corpus.num_databases} databases")

    triplets = prepare_tc_sb_triplets(
        corpus, view=cfg.views[0], target_triplets=cfg.target_triplets,
    ...
    ml = MetricsLogger(metrics_csv, strategy="TC-SB")
    ...
    logger.info(f"TC-SB model saved to {cfg.output_dir}")
